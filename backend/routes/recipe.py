...
@recipe_routes.post("/api/items", response_model=RecipeResponse, status_code=status.HTTP_201_CREATED)
def create(request: RecipeRequest, db: Session = Depends(get_db)):
    return RecipeController.create(request, db)

@recipe_routes.get("/api/items", response_model=list[RecipeResponse])
...
@recipe_routes.get("/api/items/{id}", response_model=RecipeResponse)
def find_by_id(id: int, db: Session = Depends(get_db)):
    return RecipeController.find_by_id(id, db)

@recipe_routes.delete("/api/items/{id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_by_id(id: int, db: Session = Depends(get_db)):
    return RecipeController.delete_by_id(id, db)

@recipe_routes.put("/api/items/{id}", response_model=RecipeResponse)
